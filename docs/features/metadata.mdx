...
```mermaid
flowchart LR
    A[Link saved] --> B[HTML fetch]
    B --> C[Parse OG / meta / JSON-LD]
    C --> D[Apply provider rules]
    D --> E[Upload og image to Cloudinary]
    E --> F[metadata_status: ready]
```
...
5. Upload the og/twitter image to Cloudinary (if present)
6. Set `metadata_status: ready`

If there is no og/twitter image, the feed falls back to the site favicon (or a login placeholder). Live page screenshots are not taken — they lag enrichment and often capture loading interstitials.
