...
function processActivationQueue() {
  if (processingQueue || typeof document === "undefined" || document.hidden) {
    return;
  }
    ...
    const queue = [...queuedEntries]
      ...
      .sort(
        (a, b) =>
          ...
          b.lastVisibleAt - a.lastVisibleAt,
      );
  ...
  visibilityObserver = new IntersectionObserver(
    (observed) => {
      for (const observation of observed) {
        ...
        if (!entry) continue;
        entry.visible =
          observation.isIntersecting && observation.intersectionRatio >= 0.05;
        if (entry.visible) {
          entry.lastVisibleAt = Date.now();
